...
If you are using FireWire as the fieldbus, you will need a FireWire adapter on the PC. Due to the fact that FireWire is a sophisticated protocol, some chipset implementations are not fully functional and have various issues such as dropping packets and supporting a limited number of FireWire nodes. We recommend adapters with chipsets from Texas Instruments **BUT** not all Texas Instruments chipsets work (see PC configuration in [FAQ](/jhu-dvrk/sawIntuitiveResearchKit/wiki/FAQ)).

To get the chipset model of your FireWire card, use: 
```sh
lshw
```
